...
# Norm Constraint
con = TOC.NormConstraint{Equality,Control}(m,4.0)
@test TOC.sense(con) == Equality
@test TOC.contype(con) == Control
@test evaluate(con, u)[1] == norm(u)^2 - 4
@test_throws MethodError state_dim(con)
@test control_dim(con) == m
@test TOC.check_dims(con,n,m)

con = NormConstraint{Inequality,State}(n, 2.0)
@test TOC.sense(con) == Inequality
@test TOC.contype(con) == State
...
@test TOC.is_bound(bnd)
@test TOC.upper_bound(bnd) == [x_max; u_max*Inf]
@test TOC.lower_bound(bnd) == [x_min; u_min*Inf]
...
Z = Traj(x,u,0.1,N)
@test_nowarn TOC.evaluate!(vals, bnd, Z)
@test jacobian(bnd, Z[1]) == Matrix(I,n,n+m)


# Indexed Constraint
bnd = BoundConstraint(n,m, x_max=x_max, x_min=x_min)
con = TOC.IndexedConstraint(2n,2m, bnd)
∇c = zeros(size(con))
x2 = repeat(x,2)
u2 = repeat(u,2)
z = KnotPoint(x,u,0.1)
...
@test TOC.width(con) == 2(n+m)
@test TOC.width(bnd) == n+m

con = TOC.NormConstraint{Equality,Control}(m,4.0)
∇c0 = zeros(size(con))
ix = n .+ @SVector [i for i in 1:n]
iu = m .+ @SVector [i for i in 1:m]
idx = TOC.IndexedConstraint(2n,2m, con, ix, iu)
∇c = zeros(size(idx))
@test TOC.contype(idx) == Control
@test TOC.sense(idx) == Equality
@test evaluate(idx, z2) == evaluate(con, z)
jacobian!(∇c0, con, z)
jacobian!(∇c, idx, z2)
@test ∇c == [zeros(1,m) ∇c0]

con = TOC.NormConstraint{Equality,State}(n,4.0)
idx = TOC.IndexedConstraint(2n,2m, con)
@test evaluate(idx, z2) == evaluate(con, z)
∇c0 = zeros(size(con))
∇c = zeros(size(idx))
jacobian!(∇c0, con, z)
